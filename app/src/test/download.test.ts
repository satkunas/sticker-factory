...
vi.mock('../utils/fontEmbedding', () => ({
  embedGoogleFonts: vi.fn().mockResolvedValue('@font-face { font-family: "Mock"; src: url("data:font/woff2;base64,mock"); }')
}))
...
describe('Download Output Tests', () => {
  ...
  describe('Font Embedding Integration', () => {
    it('should import font embedding utility correctly', async () => {
      const { embedGoogleFonts } = await import('../utils/fontEmbedding')
      expect(embedGoogleFonts).toBeDefined()
      expect(typeof embedGoogleFonts).toBe('function')
    })
    ...
    it('should mock font embedding behavior for testing', async () => {
      ...
      const result = await embedGoogleFonts(mockCss)
